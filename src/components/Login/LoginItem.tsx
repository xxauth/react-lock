...
export interface LoginItemProps extends Partial<FormItemProps> {
  ...
  onGetCaptcha?: (mobile: string) => Promise<boolean>;
...
const LoginItem: React.FC<LoginItemProps> = props => {
  ...
  const handleGetCaptcha = useCallback(async (mobile: string) => {
    const ret = await onGetCaptcha?.(mobile);
    if (ret) {
      setTiming(true);
    }
  }, []);
          ...
          <Row gutter={8}>
            <Col span={16}>
              <FormItem name={name} {...options}>
                <Input size="large" {...customProps} {...inputProps} />
              </FormItem>
            </Col>
            <Col span={8}>
              <Button
  ...
  return (
    <FormItem name={name} {...options}>
      <Input size="large" {...customProps} {...otherProps} />
    </FormItem>
  );
};
